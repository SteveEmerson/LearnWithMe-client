...
class GoalCard extends React.Component<GCProps,GCState>{
  ...
  updateTasks = () => {
    ...
    if (changedTasks && changedTasks.length > 0) {
      changedTasks.forEach((task: Task) => {
        const url: string = `http://${APIURL}/task/${this.props.rolePOV}_update/${task.id}`
        fetch(url, {
          method: 'PUT',
          body: JSON.stringify({
  ...
  addTasks = () => {
    ...
    if (newTasks.length > 0){
      newTasks.forEach((task) => {
        const url = `http://${APIURL}/task/${this.props.rolePOV}_create`
  ...
  removeTasks = () => {
    ...
    if (oldTasks.length > 0){
      oldTasks.forEach((task) => {
        const url = `http://${APIURL}/task/${this.props.rolePOV}_delete/${task.id}`
  ...
  updateGoal = () => {
    this.toggleEditForm()
    const url = `http://${APIURL}/goal/${this.props.rolePOV}_update/${this.props.goal.id}`
  ...
  deleteGoal = () => {
    const url=`http://${APIURL}/goal/${this.props.rolePOV}_delete/${this.props.goal.id}`
    fetch(url, {
      method: 'DELETE',
      headers: new Headers({
        'Content-Type': 'application/json',
